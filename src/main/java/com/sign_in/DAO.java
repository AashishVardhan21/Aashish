...
public class DAO extends scan {
	public static String mobile (String num)
	{
		...
		et.begin();
		Query q=em.createQuery("select e.number from DTO e where e.number=?1");
	...
	public static String password(String mNumber) {
		...
		EntityTransaction et=em.getTransaction();
		et.begin();
		Query q=em.createQuery("select e.password from DTO e where e.number=?1 ");
	...
	static int i=0;
	public static void forgotPass(String mNumber) {
		EntityManagerFactory emf=Persistence.createEntityManagerFactory("project");
		EntityManager em=emf.createEntityManager();
		EntityTransaction et=em.getTransaction();
		...
		if(com.sign_up.Validation.validatepass(newPass)) {
			String old_password=DAO.password(mNumber);
			
			if(newPass.equals(old_password)) {
				System.out.println("Old passkey detected, please enter new passkey");
				forgotPass(mNumber);
			}
			else {
				et.begin();
				Query q=em.createQuery("update DTO e set e.password=?1 where e.number=?2");
				...
				if(i>0)
				{
					System.out.println("Passkey succesfully updated");
					Sign_in.signin();
				}
				else {
					System.out.println("Passkey not updated");
					forgotPass(mNumber);
				}
			}
			...
			System.out.println("Passkey mismatch");
			forgotPass(mNumber);
		}
	...
	public static int otp(String moNumber) {
		...
		EntityTransaction et=em.getTransaction();
		et.begin();
		Query q = em.createQuery("SELECT e.otp FROM DTO e WHERE e.number = :mobile");
	...
	public static void readAllDats(int id) {
		...
		EntityTransaction et=em.getTransaction();
			...
			et.begin();
			Query q=em.createQuery("select e from DTO e where e.id=?1");
			q.setParameter(1, id);
	...
	public static int getId(String mNumber) {
		...
		// TODO Auto-generated method stub
				...
				EntityTransaction et=em.getTransaction();
					...
					et.begin();
					Query q=em.createQuery("select e.id from DTO e where e.number=?1");
					q.setParameter(1, mNumber);
	...
	public static void deleteUserAccount(int id) {
		...
		try {
			...
			et.begin();
			Query q=em.createQuery("delete from DTO e where e.id=?1");
			q.setParameter(1, id);
			if(q!=null)
			{
	...
	static String s="update Dto a set ";
